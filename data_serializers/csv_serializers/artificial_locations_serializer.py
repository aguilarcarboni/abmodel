...
CSV_FILE_NAME = 'nakamura_1983_ai_locations.csv'
CSV_EXPORT_NAME = 'artificial_moonquake_locations.json'
CSV_EXPORT_COLS = {
    'label': 'AI',
    'year': 'Y',
    'day': 'JD',
...
def calculate_date(row):
    ...
    date = date.replace(hour=row['hour'], minute=row['minutes'], second=int(row['seconds']))
    return date.isoformat()

def load_artificial_locations_csv_file(data_files_path = DATA_FILES_PATH):
...
if __name__ == '__main__':
    load_artificial_locations_csv_file()
